...
class LogStash::Filters::Geocode < LogStash::Filters::Base
  ...
  def register
    ...
    if @database.nil?
      @database = LogStash::Environment.vendor_path("locations.csv")
      if !File.exists?(@database)
        raise "You must specify 'database => ...' in your geocode filter (I looked for '#{@database}'"
      end
    end
...
end # class LogStash::Filters::Geocode
